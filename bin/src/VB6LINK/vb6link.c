/*
	...
	The purpose of the wrapper is to link native Visual Basic 6 applications to 32-bit DLL,
	especially when using non-stdcall calling conventions (e.g. cdecl), by injecting
	import libraries generated by the ImpLib SDK into the Microsoft linker command line.

	How to install this wrapper:
	----------------------------
	...
	the Visual Basic 6 project files are located. For each import library create a module
	file with the same name as the DLL. For example, for OPENAL32.DLL the import library
...
int c, i = ep();
	if(copy_LNK)
		for(c = 0; c < 8; c++){
			Sleep(128);
			if(DeleteFile("LNK.EXE")) // Try to delete LNK.EXE
				break;
		}
	ExitProcess(i);
}
